...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const requestedSeats = Number(request.no_of_seats || 0);
      const availableSeats = Number(request.seats_available || 0);
      ...
      if (action === "accept") {
        ...
        await conn.query(
          `
          UPDATE rides 
          SET seats_available=?, ride_status=? 
          WHERE id=?
          `,
